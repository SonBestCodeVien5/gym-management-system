...
- **Công thức:** Tiền Hoàn = `remainingSessions * unitPrice`.
- **Quy tắc thời gian:** Hoàn 50% trong 72h đầu nếu chưa tập; Hoàn 20% giá trị còn lại nếu đã tập dưới hoặc bằng 50% số buổi; Không hoàn nếu tập > 50%.
...
| GET | /api/v1/branches/nearby | Tìm chi nhánh theo tọa độ GPS (planned). |
| POST | /api/v1/subscriptions/:id/refund | Yêu cầu hoàn tiền (planned). |
| POST | /api/v1/auth/login | Dang nhap (planned). |
| POST | /api/v1/auth/refresh | Cap lai access token (planned). |
| POST | /api/v1/auth/logout | Dang xuat (planned). |
