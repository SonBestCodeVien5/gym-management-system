...
|---|---:|---|---|
| Missing unique index on `refunds.subscription_id` | medium | DB does not enforce one refund per subscription. Current service pre-check + atomic subscription update prevents normal double refund, but data integrity still depends on app logic. Manual DB writes or future code paths could create duplicate refund records. | Soon, before production or before more refund flows. Good fit for `07_indexes_data_integrity`. |
