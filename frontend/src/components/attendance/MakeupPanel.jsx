...
const INITIAL_VALUES = { subscription_id: '', branch_id: '', date: '', is_makeup_for: '' }
...
function validate(values) {
  ...
  if (!values.is_makeup_for || Number.isNaN(new Date(values.is_makeup_for).getTime())) {
    errors.is_makeup_for = 'Source missed date is required.'
  }

  return errors
}

function MakeupPanel({ accessToken, branches = [], subscriptionId = '', onSuccess, branchListId = 'makeup-branch-options' }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      await createMakeupAttendance(accessToken, cleanAttendancePayload(values))
      setState({ status: 'success', error: null, notice: 'Makeup attendance recorded.' })
      onSuccess?.()
    } catch (error) {
  ...
  return (
    <DataPanel title="Makeup attendance">
      <form className="resource-form" onSubmit={handleSubmit}>
        <div className="resource-form__grid">
          ...
          <div className="field-group">
            ...
            <input id="makeup-date" type="datetime-local" value={values.date} onChange={(event) => updateField('date', event.target.value)} />
          </div>
          <div className="field-group">
        ...
        <div className="resource-form__actions">
          <button className="btn-primary" type="submit" disabled={state.status === 'submitting'}>{state.status === 'submitting' ? 'Recording' : 'Record makeup'}</button>
        </div>
      </form>
      {state.error ? <div className="form-alert" role="alert">{apiErrorText(state.error, 'Makeup failed.')}</div> : null}
